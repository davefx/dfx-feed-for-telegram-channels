...
?>
<div class="dfx-tg-feed-browser dfx-tg-feed-layout<?php echo !empty($wrapper_class) ? ' ' . esc_attr($wrapper_class) : ''; ?>"><?php foreach ($messages as $msg): ?>
        <?php
        ...
        ?>
        <article class="dfx-tg-feed-message dfx-tg-feed-message-browser <?php echo $is_sticker ? 'dfx-tg-feed-sticker' : ''; ?>" data-id="<?php echo esc_attr($msg['id']); ?>">
            <!-- 1. Date (first) -->
            ...
            <?php if (!empty($msg['media'])): ?>
                <div class="dfx-tg-feed-media dfx-tg-feed-media-browser <?php echo $is_sticker ? 'dfx-tg-feed-media-sticker' : ''; ?>">
                    <?php 
                    $sticker_type = $msg['sticker_type'] ?? null;
                    if ($is_sticker && $sticker_type === 'tgs'): 
                        // TGS (Lottie) animated sticker
                    ?>
                        <div class="dfx-tg-sticker-container dfx-tg-sticker-browser" data-sticker-url="<?php echo esc_url($msg['media']); ?>" data-file-id="<?php echo esc_attr($msg['file_id'] ?? ''); ?>"></div>
                        <?php if ($has_emoji): ?>
                    ...
                    <?php elseif ($is_sticker && $sticker_type === 'webm'): 
                        // WEBM video sticker
                    ?>
                        <video class="dfx-tg-sticker-video dfx-tg-sticker-browser" autoplay loop muted playsinline>
