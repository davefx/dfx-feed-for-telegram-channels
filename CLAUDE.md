...
3. **Smart on-demand refresh** (in `Shortcodes::shortcode_channel_feed`): on each frontend render, if the `dfxtgfeed_last_sync_{channel}` transient is older than 10 minutes and no `dfxtgfeed_refreshing_{channel}` lock is held, it triggers a refresh inline. The lock transient prevents thundering-herd refreshes.
