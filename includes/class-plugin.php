...
final class Plugin {
    ...
    const MENU_SLUG = 'dfxfftc-messages';

    /** @var string Cron hook name for the periodic background refresh */
    const CRON_HOOK = 'dfxfftc_cron_refresh';

    /** @var string Custom cron schedule slug */
    const CRON_SCHEDULE = 'dfxfftc_two_minutes';
    ...
    private function init() {
        ...
        add_action('wp_ajax_dfxfftc_test', [Settings::instance(), 'ajax_test_bot_channel']);
        add_action('wp_ajax_dfxfftc_reload', [Settings::instance(), 'ajax_reload_messages']);
        add_action('wp_ajax_dfxfftc_refresh', [PostType::instance(), 'ajax_refresh_messages']);
        add_action('wp_ajax_dfxfftc_proxy_sticker', [$this, 'ajax_proxy_sticker']);
        add_action('wp_ajax_nopriv_dfxfftc_proxy_sticker', [$this, 'ajax_proxy_sticker']);
        add_action('wp_ajax_dfxfftc_proxy_media', [$this, 'ajax_proxy_media']);
        add_action('wp_ajax_nopriv_dfxfftc_proxy_media', [$this, 'ajax_proxy_media']);
    ...
    public static function media_proxy_url($file_id) {
        ...
        return add_query_arg([
            'action'  => 'dfxfftc_proxy_media',
            'file_id' => $file_id,
            'nonce'   => wp_create_nonce('dfxfftc_media_proxy'),
    ...
    public function cron_refresh_callback() {
        ...
        $channels = $wpdb->get_col(
            "SELECT DISTINCT meta_value FROM {$wpdb->postmeta}
             WHERE meta_key = '_dfxfftc_channel' AND meta_value != ''"
        );
        if (empty($channels)) {
            return;
        }

        $limit = max(10, (int) get_option('dfxfftc_default_count', 10));
        foreach ($channels as $channel) {
            $channel_safe = sanitize_key($channel);
            if (get_transient("dfxfftc_refreshing_{$channel_safe}")) {
                continue;
            }
            set_transient("dfxfftc_refreshing_{$channel_safe}", true, MINUTE_IN_SECONDS);
            try {
                PostType::instance()->refresh_messages($channel, $limit);
                set_transient("dfxfftc_last_sync_{$channel_safe}", time(), HOUR_IN_SECONDS);
            } catch (\Throwable $e) {
    ...
    public function settings_page() {
        // Add top-level menu
        add_menu_page(
            __('Telegram Messages', 'dfx-feed-for-telegram-channels'),
            __('Telegram Messages', 'dfx-feed-for-telegram-channels'),
            'manage_options',
            self::MENU_SLUG,
        ...
        add_submenu_page(
            self::MENU_SLUG,
            __('All Messages', 'dfx-feed-for-telegram-channels'),
            __('All Messages', 'dfx-feed-for-telegram-channels'),
            'edit_posts',
            'edit.php?post_type=dfxfftc_message'
        );
        
        // Add settings submenu under the top-level menu
        $settings_hook = add_submenu_page(
            self::MENU_SLUG,
            __('Settings', 'dfx-feed-for-telegram-channels'),
            __('Settings', 'dfx-feed-for-telegram-channels'),
            'manage_options',
            'dfxfftc',
            [Settings::instance(), 'render_page']
        );
        add_action('admin_print_scripts-' . $settings_hook, [Settings::instance(), 'enqueue_settings_scripts']);
    }
    ...
    public function redirect_to_messages() {
        wp_redirect(admin_url('edit.php?post_type=dfxfftc_message'));
        exit;
    }
    ...
    public function ajax_proxy_sticker() {
        ...
        $cache_key = 'dfxfftc_sticker_' . md5($file_id);
    ...
    private function media_cache_paths($file_id) {
        ...
        $dir = trailingslashit($upload['basedir']) . 'dfxfftc-cache';
        if (!file_exists($dir)) {
            wp_mkdir_p($dir);
    ...
    /**
     * One-time DB migration. Idempotent and gated by `dfxfftc_db_version`.
     *
     * Handles upgrades from two legacy prefix eras:
     *   - Pre-1.0.0: `dfx_tg_message` post type, `_tg_*` meta, `dfx_tg_feed_*` options.
     *   - v1.0.0–1.0.2: `dfxtgfeed_message` post type, `_dfxtgfeed_*` meta, `dfxtgfeed_*` options.
     * Both get migrated to the current `dfxfftc_*` prefix.
     *
     * Also scrubs any leftover token-bearing media URLs.
     */
    private function maybe_run_db_migration() {
        $target_version = '4';
        if (get_option('dfxfftc_db_version') === $target_version) {
            return;
        }

        global $wpdb;

        // --- Legacy era 1: pre-1.0.0 prefixes ---

        $wpdb->query(
            "UPDATE {$wpdb->posts} SET post_type = 'dfxfftc_message'
             WHERE post_type = 'dfx_tg_message'"
        );
        $wpdb->query(
            "UPDATE {$wpdb->postmeta}
             SET meta_key = CONCAT('_dfxfftc_', SUBSTRING(meta_key, 5))
             WHERE meta_key LIKE '\\_tg\\_%'"
        );

        $legacy_options = [
            'dfx_tg_feed_bot_token'      => 'dfxfftc_bot_token',
            'dfx_tg_feed_default_count'  => 'dfxfftc_default_count',
            'dfx_tg_feed_channel'        => 'dfxfftc_channel',
            'dfx_tg_feed_db_version'     => 'dfxfftc_db_version',
        ];
        ...
        // --- Legacy era 2: v1.0.0–1.0.2 (dfxtgfeed prefix) ---

        $wpdb->query(
            "UPDATE {$wpdb->posts} SET post_type = 'dfxfftc_message'
             WHERE post_type = 'dfxtgfeed_message'"
        );
        $wpdb->query(
            "UPDATE {$wpdb->postmeta}
             SET meta_key = REPLACE(meta_key, '_dfxtgfeed_', '_dfxfftc_')
             WHERE meta_key LIKE '\\_dfxtgfeed\\_%'"
        );

        $v1_options = [
            'dfxtgfeed_bot_token'      => 'dfxfftc_bot_token',
            'dfxtgfeed_default_count'  => 'dfxfftc_default_count',
            'dfxtgfeed_channel'        => 'dfxfftc_channel',
            'dfxtgfeed_db_version'     => 'dfxfftc_db_version',
        ];
        foreach ($v1_options as $old => $new) {
            $value = get_option($old, null);
            if ($value !== null) {
                if (get_option($new, null) === null) {
                    update_option($new, $value);
        ...
        // --- Scrub token-bearing media URLs from any era ---

        $wpdb->query(
            "UPDATE {$wpdb->postmeta} SET meta_value = '1'
             WHERE meta_key = '_dfxfftc_media'
               AND meta_value LIKE 'https://api.telegram.org/%'"
        );

        update_option('dfxfftc_db_version', $target_version);
    }
}