...
class Shortcodes {
    ...
    public function register() {
        add_shortcode('dfxtgfeed_channel_feed',    [$this, 'shortcode_channel_feed']);
        add_shortcode('dfxtgfeed_channel_browser', [$this, 'shortcode_channel_browser']);
    ...
    public function enqueue_styles() {
        ...
        wp_enqueue_script(
            'dfxtgfeed-stickers',
            DFXTGFEED_URL . 'assets/js/stickers.js',
            ['lottie-player'],
            DFXTGFEED_VER,
            true
        );
        ...
        wp_localize_script('dfxtgfeed-stickers', 'dfxtgfeedStickers', [
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('dfxtgfeed_sticker_proxy')
        ]);
        
        // Enqueue lightbox script for image viewing
        wp_enqueue_script(
            'dfxtgfeed-lightbox',
            DFXTGFEED_URL . 'assets/js/lightbox.js',
            [],
            DFXTGFEED_VER,
            true
        );
    ...
    private function load_template($template_name, $variables = []) {
        extract($variables);
        $template_path = DFXTGFEED_PATH . 'templates/' . $template_name . '.php';
        
        // Allow themes to override templates
        $theme_template = locate_template('dfxtgfeed/' . $template_name . '.php');
        if ($theme_template) {
    ...
    // [dfxtgfeed_channel_feed channel="tele_channel" count="5"]
    public function shortcode_channel_feed($atts) {
        $a = shortcode_atts([
            'channel' => '',
            'count'   => get_option('dfxtgfeed_default_count', 10),
        ], $atts );
        ...
        $last_sync = get_transient("dfxtgfeed_last_sync_{$channel_safe}");
        if (!$last_sync || (time() - $last_sync) > 600) { // 600 seconds = 10 minutes
            ...
            if (!get_transient("dfxtgfeed_refreshing_{$channel_safe}")) {
                set_transient("dfxtgfeed_refreshing_{$channel_safe}", true, 30); // 30-second lock
                PostType::instance()->refresh_messages($channel, 100);
                set_transient("dfxtgfeed_last_sync_{$channel_safe}", time(), 3600);
        ...
        $wrapper_class = apply_filters('dfxtgfeed_wrapper_class', '');
    ...
    // [dfxtgfeed_channel_browser channel="tele_channel"]
